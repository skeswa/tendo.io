    ...
    (function($) {
        $.fn.JSNESUI = function() {
            ...
            var UI = function(nes) {
                ...
                JSNES.UIDelegate.isPaused = function () {
                    return !(self.nes.isRunning);
                };

                JSNES.UIDelegate.pause = function() {
                ...
                JSNES.UIDelegate.resume = function() {
                    if (!(self.nes.isRunning)) {
                        self.nes.start();
                    } 
                };

                JSNES.UIDelegate.load = function(url) {
                    self.loadROM(url);
                };

                JSNES.UIDelegate.restart = function() {
                    self.nes.reloadRom();
                    self.nes.start();
                };

                JSNES.UIDelegate.isSoundEnabled = function () {
                    return !!(self.nes.opts.emulateSound);
                };

                JSNES.UIDelegate.enableSound = function () {
                    if (!(self.nes.opts.emulateSound)) {
                        self.nes.opts.emulateSound = true;
                    }
                };

                JSNES.UIDelegate.disableSound = function () {
                    if (self.nes.opts.emulateSound) {
                ...
                self.zoomed = false;
                JSNES.UIDelegate.isZoomed = function () {
                    return self.zoomed;
                };

                JSNES.UIDelegate.zoom = function () {
                    if (!self.zoomed) {
                        self.screen.animate({
                            width: '512px',
                            height: '480px'
                ...
                JSNES.UIDelegate.unzoom = function () {
                    if (self.zoomed) {
                        self.screen.animate({
                            width: '256px',
                            height: '240px'
